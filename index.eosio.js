...
const client = createDfuseClient({
  ...
  network: 'localhost:13019',
  secure: false
});
// CODE:END:quickstarts_javascript_node_eos_section1
// CODE:BEGIN:quickstarts_javascript_node_eos_section2
// //   searchTransactionsForward(query:"receiver:token action:transfer -data.quantity:'0.0001 EOS'", cursor: $cursor) {
// searchTransactionsForward(query:"receiver:ingestion action:process", cursor: $cursor) {
  ...
  const stream = await client.graphql(operation, message => {
    if (message.type === 'data') {
      ...
      matchingActions.forEach(({ json: { from, to, quantity } }) => {
        console.log(
          `Detected Event: => Transfer ${from} -> ${to} [${quantity}]${undo ? ' REVERTED' : ''}`
        );
      });
